...
io.on("connection", (socket) => {
  ...
  socket.on("user-online", async (userId) => {
    try {
      ...
      activeUsers.set(userId, { socketId: socket.id, lastHeartbeat: Date.now(), heartbeatTimeout })

      // ✅ join stable per-user room
  ...
  socket.on("heartbeat", (userId) => {
    ...
    if (u) {
      ...
      activeUsers.set(userId, { socketId: u.socketId, lastHeartbeat: Date.now(), heartbeatTimeout: newTimeout })
    }
  })
  ...
  // ✅ FIX: emit to recipient AND sender
  ...
  socket.on("ice-candidate", (data) => {
    ...
    const r = activeUsers.get(recipientId)
    if (r) io.to(r.socketId).emit("ice-candidate", candidate)
  })
...
cron.schedule("*/5 * * * *", async () => {
  try {
    ...
    const staleUsers = await User.find({ isOnline: true, lastSeen: { $lt: thirtyMinutesAgo } })
